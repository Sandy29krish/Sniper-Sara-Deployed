...
def start_command(message): STATUS["active"] = True STATUS["mode"] = "LIVE" send_telegram_message("✅ Sniper bot started. Monitoring live market conditions...")

@bot.message_handler(commands=["stop_sniper"]) 
def stop_command(message): STATUS["active"] = False STATUS["mode"] = "STOPPED" send_telegram_message("🛑 Sniper bot stopped by user.")
...
def pnl_command(message): try: with open("trade_log.json", "r") as f: data = f.read() send_telegram_message(f"📊 Today's Trade Log:\n{data[-4000:]}") except: send_telegram_message("⚠️ No trade log found or error reading.")
